...
inputMoney = int(input(f'총 금액은 {money}원입니다. 돈을 투입해주세요 >>> '))
cnt = 0
while (1):
  ...
  if inputMoney < money:
    if cnt == 3:
        ...
        break
    putMoney = int(input(f'{money-inputMoney}원이 부족합니다. 돈을 추가로 투입해주세요 >>> '))
    money -= putMoney
    cnt += 1
  # 한번에 성공시 거스름돈 구현
  else:
    if cnt == 0:
      print(f'{inputMoney}원을 받았습니다. 거스름돈은 {inputMoney - money}입니다.')
      money -= inputMoney
      break
    else:
      print(f'{putMoney}원을 받았습니다. 거스름돈은 {inputMoney - money}입니다.')
      money -= inputMoney
      break

if money <= 0:
  print(f'주문하신 {coffee_kind_dict[coffee_kind]} 나왔습니다.')
  for i in range(many_coffee):
    printCofee()
